...
def get_logger(name, log_dir, config_dir, epoch):
    ...
    """
    # logging.basicConfig(filename='./log/wn.txt', filemode='a', level=logging.INFO, format='%(message)s')
    config_dict = json.load(open(config_dir + 'log_config.json'))
    config_dict['handlers']['file_handler']['filename'] = log_dir + name.replace('/', '-') + "_" + str(
        epoch) + "epoch" + ".txt"
    ...
    consoleHandler.setFormatter(logging.Formatter(fmt=std_out_format, datefmt=date_format))
