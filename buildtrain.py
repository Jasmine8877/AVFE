...
def evaluate(model, x_test, batch_size, target_dict):
    ...
    batch_num = math.floor(len(x_test) / batch_size)
    tail_scores_all = []
    tail_label = []
...
def train_epoch(train_doubles, num_batches_per_epoch, batch_size, model, opt, scheduler, x_valid, target_dict, num, device, max_mrr=0, epoch=1000,
    ...
    for epoch in range(epoch):
        model.train()
        random.shuffle(train_doubles) # 173670
        losses = []
        for batch_num in range(num_batches_per_epoch): # 169    0-168
            opt.zero_grad()
            x_batch = np.array(train_doubles[batch_num * batch_size:(batch_num + 1) * batch_size]) # 172032 - 173056
            # print("xbatch:",(batch_num + 1) * batch_size-batch_num * batch_size)
            batch_h, batch_r, batch_t = x_batch[:, 0], x_batch[:, 1], x_batch[:, 2]
            # print("batch_h,batch_r",batch_h,batch_r)
            e1 = batch_h
            rel = batch_r
            e2_multi = batch_t
            # cnt + +s
