...
class PyramidE(torch.nn.Module):
    def __init__(self, logger, num_emb, embedding_dim=200, input_drop=0.5, hidden_drop=0.5, feature_map_drop = 0.5,k_w = 10, k_h = 20,
        ...
        self.se1 = SELayer(self.kernel_outchannel[0], ratio = int(0.5*output_channel),kernel_size=3)
        self.se3 = SELayer(self.kernel_outchannel[1], ratio = int(0.5*output_channel),kernel_size=3)
        self.se5 = SELayer(self.kernel_outchannel[2], ratio = int(0.5*output_channel),kernel_size=3)
        self.SK = SKConv(self.kernel_outchannel[0])
        n_head = 8
        print("n_head:",n_head)
    ...
    def forward(self, e1, rel):
        ...
        x = torch.cat([x_1, x1, x2, x_2, x3], dim=1)  #1,5,5,15,5 1024,17,20,20 不加x_origin
